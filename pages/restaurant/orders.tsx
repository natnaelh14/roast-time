...
const Orders = ({ token, accountId, restaurant }: { token: string; accountId: string; restaurant: Restaurant }) => {
		...
		token
			? [
					`${process.env.NEXT_PUBLIC_BASE_URL}/reservations/${accountId}/restaurant/${restaurant?.id}/${
						dayjs(reservationDate).isValid() ? dayjs(reservationDate).add(1, "days").utc().format() : ""
					}`,
					token,
			  ]
			: null,
		...
		<div className="flex min-h-[700px] justify-center">
			<div className="m-8 h-fit w-full rounded-md border-gray-200 bg-white p-8 shadow-xl dark:border-gray-secondary dark:bg-blue-dark lg:w-2/3">
				<div className=" flex items-center justify-between pb-6">
					<div>
						<h2 className="font-semibold text-brown-dark md:text-3xl">{restaurant?.name}</h2>
						<span className="dark:text-brown-light md:text-xl">All reservations</span>
					</div>
					<DatePicker
						label="Filter Date"
								...
								<table className="min-w-full leading-normal">
									...
									<tbody className="bg-gray-100 dark:bg-blue-light">
										{reservations.map((reservation: Reservation) => {
											return <OrderItem key={reservation.id} reservation={reservation} mutate={mutate} />;
										})}
									</tbody>
								</table>
							</div>
